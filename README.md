...
Тесты на **Vitest** отделены от production-кода:

- `tests/unit/` — чистая логика и локальные DOM-контракты;
- `tests/integration/` — lifecycle формы, renderer и session state;
- `tests/fixtures/` и `tests/helpers/` — общие тестовые данные и DOM helpers.
...
Покрыты нормализация и валидация формы, условная видимость опций, session state,
DOM/ARIA renderer, lifecycle, сопоставление отелей и payload лида. Анимации и
визуальные тайминги проверяются вручную в браузере после `npm run dev`.
